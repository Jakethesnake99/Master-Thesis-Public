...
Run_sim_func = function(par_comb_df,fixed_parameters, B) {
  ...
  #send functions and packages to cores
  ...
  #run the function 1000 times for each scenario and neatly store the results ALSO GENERATES RELATIVE RESULTS.
  result_list = parLapply(cl,1:nrow(par_comb_df), function(i) {
    ...
    result = result %>%
      ...
      mutate( rarmse = (armse /armse[estimation_method == "MLE_ps"])-1,
              rMoAB = (moab /moab[estimation_method == "w"])-1)
    
  
    list(result = result, MSE_agg_df = output$MSE_df, hyper_agg_df = output$hyper_comb_df)
  })

#just bind the results from each scenario to a main big df       
